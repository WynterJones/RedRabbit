...
const unlink$1 = util.promisify(fs.unlink);
...
class Config {
  static async new(state, defaultSchema) {
...
async function check(state) {
  ...
  const config = await configure(state);
  // get the signature if we haven't provided one
  if (state.signature === '') {
    state.signature = (await config.get('signature')) || v4_1();
  }
  // format the URL
